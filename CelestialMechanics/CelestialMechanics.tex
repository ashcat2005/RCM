...
\end{equation}
where $\phi(t, \textbf{x})$ represents the \textit{gravitational potential} depending on time and space coordinates $\textbf{x} = (x,y,z)$, $G = 6.67 \times 10^{-11} \textrm{ N m}^2\textrm{/kg}^2$ is the Newtonian gravitational constant and $\rho(t, \textbf{x})$ is the matter density. As is shown in Appendix \ref{A:GreenFunctionPoisson}, a general solution of this equation can be written in terms of the corresponding Green function as
\begin{equation}
...
In order to obtain the conserved quantities in the two-body system description, we will consider the usual spherical coordinates $\textbf{x} = (r, \theta, \varphi )$, in which the Lagrangian (\ref{eq:2BodyLagrangian}) takes the form
\begin{equation}
...
\begin{equation}
p_\varphi = \frac{\partial L}{\partial \dot{\varphi}} = \mu r^2 \sin^2 \theta \dot{\varphi} \equiv \mu \ell _z, \label{eq:conservedMomentum1}
\end{equation}
and it is identified with the azimuthal component of the total angular momentum. 
Similarly, it is also easy to see that the Hamiltonian 
\begin{equation}
H = \sum_{j=1} ^3 p_j \dot{x}^j - L  = \frac{1}{2} \mu \dot{\textbf{x}}^2 - \frac{GM\mu }{r} \label{eq:2BodyHamiltonian}
\end{equation}
is also conserved and that it corresponds to the total energy of the system.


The total angular momentum of the two-body system, assuming that the particles have no spin, is given by $\mu \boldsymbol{\ell} = m_1 \textbf{x}_1 \times \dot{ \textbf{x} }_1 + m_2 \textbf{x}_2 \times \dot{ \textbf{x} }_2 = \mu \textbf{x} \times \dot{ \textbf{x} }$. The quantity $\boldsymbol{\ell} = \textbf{x} \times \dot{\textbf{x}}$ is known as the specific angular momentum (i.e. angular momentum per unit mass) and it is also a constant of motion, i.e. $\frac{d\boldsymbol{\ell}}{dt}=0$, due to the fact that the acceleration lies in the direction $\textbf{x}$, as shown in equation (\ref{eq:2BodyEoM}). 

Since $\boldsymbol{\ell}$ is constant and orthogonal to both position and velocity vectors, $\boldsymbol{\ell}\cdot \textbf{x}  = \boldsymbol{\ell} \cdot \dot{\textbf{x}} = 0$, the motion of the system lies in a fixed plane that is always perpendicular to the total angular momentum. Introducing a coordinate system with unit vectors $\{ \textbf{i}, \textbf{j}, \textbf{k} \}$  in which the orbit lies in the $XY$ plane, we take $\boldsymbol{\ell}$ along the $Z$-axis,
\begin{equation}
\boldsymbol{\ell} = \textbf{x} \times \dot{ \textbf{x}} = \ell \textbf{k}, \label{eq:2BodyAngularMomentum}
\end{equation}
$\ell = \left| \boldsymbol{\ell} \right| = \left| \textbf{x} \times \dot{\textbf{x}} \right|$ and $\textbf{k}$ is the unit vector in the $Z$-direction. The motion in this plane can be described by taking $\theta=\frac{\pi}{2}$ and using only the polar coordinates $r$ and $\varphi$. Hence, the position vector has components $\textbf{x} = (r \cos \varphi , r \sin \varphi , 0)$, where we are measuring the angle $\varphi$ with respect to the $X$-axis, as usual. 
...
\end{equation}
as can be proben from equation (\ref{eq:conservedMomentum1}. Another conserved quantity is the \textit{Laplace-Runge-Lenz vector} (also called the \textit{eccentricity vector} for reasons that we will show below), defined as 
\begin{equation}
\textbf{e} = \frac{\dot{\textbf{x}} \times \boldsymbol{\ell}}{GM} - \textbf{n}. \label{eq:2BodyLaplaceVector}
\end{equation}

Due to its definition, it is clear that 
\begin{equation}
...
\subsection{Equation of the Trajectory}
...
\end{equation}
where we have defined the total energy per unit mass $\mathcal{E} = \frac{H}{\mu}$. Introducing an effective potential energy,
\begin{equation}
V_{\text{eff}} (r) = \frac{\ell^2}{2r^2}  - \frac{GM}{r},
\end{equation}
the radial equation can be written as 
\begin{equation}
\frac{1}{2} \dot{r}^2 = \mathcal{E} - V_{\text{eff}} (r). \label{eq:2BodyEoM1}
\end{equation}

It is possible to obtain information about the trajectories by studying  the function $V_{\text{eff}} (r)$. For example, from the plot in Figure \ref{fig:2BodyEffectivePotential} one can note that there exist circular orbits, determined by the condition
\begin{equation}
\left. \frac{dV_{\text{eff}}}{dr} \right|_{r=r_c} = 0.
\end{equation}
By solving this relation for a given value of $\ell$, we obtain the radius of the circular orbit as $r_c = \frac{\ell^2}{GM}$. Replacing this radius in equation (\ref{eq:2BodyEnergy}), we obtain the energy of the particle moving in the circular orbit,
\begin{equation}
...
In order to describe more general orbits, the analytic form of the trajectories can be found by considering $\varphi$ as the independent variable (instead of the time) and writing $r=r(\varphi)$.  

\begin{exercise}[Differential Equation of the Trajectory]
\textbf{Differential Equation of the Trajectory}

Introduce a new auxiliar variable, $u(\varphi)=\frac{1}{r(\varphi)}$. Show that the second order differential equation of the trajectory given in equation (\ref{eq:2BodyEoM1}), is writen in terms of $u(\varphi)$ as
\begin{equation}
...
r_{p} = \frac{p}{1+e}
\end{equation}
while $\varphi=\omega + \pi$ gives the maximum value of the radius (\textit{apocenter}),
\begin{equation}
 r_{a} = \frac{p}{1-e}.
 \end{equation} 
...
\end{equation}
where the minus sign in the value of the energy denotes that the motion is bound. Similarly, the angular momentum vector per unit mass in equation (\ref{eq:2BodyAngularMomentum}) can be written as
\begin{equation}
...
Show that the Laplace-Runge-Lenz vector (\ref{eq:2BodyLaplaceVector}), for elliptic trajectories, can be written as
\begin{equation}
...
\begin{exercise}[Two Body System: Numerical Solution]
 \textbf{Two Body System: Numerical Solution}
 
 Using the barycentric frame and the cartesian coordinates $\textbf{x} = (x^1, x^2, x^3)$, write a code that solves the equations of motion (\ref{eq:2BodyEoM}) using an adaptive step Runge-Kutta 4/5 integrator. Include a function that verifies that the conserved quantities (energy and angular momentum) remain constant along the integration domain. Also include a function that plots the obtained orbit in 3-dimensional space.

 As a particular example, integrate the motion of Earth around the Sun using the astronomical data available in Appendix \ref{A:AstronomicalData}.
...
\end{equation}
where $t_p$ corresponds to the \textit{time of pericenter passage}. Meanwhile, using equation (\ref{eq:2BodyAngularMomentum}), it is possible to write the angular velocity as
 \begin{align}
...
We begin this approach by defining two new variables, the \textit{true anomaly} $f = \varphi - \omega$ and the \textit{eccentric anomaly} $E$. Figure \ref{fig:ReferenceCircleandOrbit} illustrates these angular variables and, based on the geometry of the plot, it is possible to show that these variables are related through the relations
 ...
 \end{align}
 \end{subequations}
These expressions give the relation
 \begin{equation}
...
l = \sqrt{\frac{GM}{a^3}} \left( t-t_p \right) =  \left( E - e \sin E \right), \label{eq:2BodyKeplerEquation}
 \end{equation}
where we introduced the \textit{mean anomaly}, $l$. Using this equation, we can obtain the position at each time as follows: 
\begin{enumerate}
\item Given a time $t$, we invert Kepler's equation (\ref{eq:2BodyKeplerEquation}) to obtain the eccentric anomaly $E(t)$.
...
Thus, there are six \textit{orbital elements} or parameters that define completely the orbit in 3-dimensional space,
\begin{itemize}
...
& + \left( \sin \iota \sin \omega \right) \textbf{e}_z \left. \right].
 ...
 \begin{exercise}[Sun-Jupiter System]
...
\begin{exercise}[Earth-Moon System]
